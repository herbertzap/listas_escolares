...
function buscarRegiones(termino) {
  const terminoLower = termino.toLowerCase();
  return chileData.regiones.filter(region => 
    region.nombre.toLowerCase().includes(terminoLower)
  );
}

// Función para buscar comunas por región
function buscarComunasPorRegion(regionId, termino = '') {
  const region = chileData.regiones.find(r => r.id === regionId);
  if (!region) return [];
  
  if (!termino) return region.comunas;
  
  const terminoLower = termino.toLowerCase();
  return region.comunas.filter(comuna => 
    comuna.toLowerCase().includes(terminoLower)
  );
}

// Función para obtener todas las regiones
function obtenerTodasLasRegiones() {
  return chileData.regiones.map(region => ({
    id: region.id,
    nombre: region.nombre
  }));
}

// Función para obtener todas las comunas de una región
function obtenerComunasDeRegion(regionId) {
  const region = chileData.regiones.find(r => r.id === regionId);
  return region ? region.comunas : [];
}

// Función para buscar comunas en todo Chile
function buscarComunasEnTodoChile(termino) {
  const terminoLower = termino.toLowerCase();
  const resultados = [];
  
  chileData.regiones.forEach(region => {
    const comunasEncontradas = region.comunas.filter(comuna => 
      comuna.toLowerCase().includes(terminoLower)
    );
    
    if (comunasEncontradas.length > 0) {
      resultados.push({
        region: region.nombre,
        comunas: comunasEncontradas
      });
    }
  });
